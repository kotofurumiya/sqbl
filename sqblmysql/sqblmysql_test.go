...
func TestSqblMysql_BuildSQL(t *testing.T) {
	t.Parallel()
	...
	t.Run("build common SQL", func(t *testing.T) {
		...
		expected := sqltesting.Compact(
			"SELECT " +
				...
				"WHERE `p`.`purchased_at` >= '2025-01-01' AND (`u`.`status` = 'active' OR `u`.`role` = 'admin') " +
				"GROUP BY `u`.`id`, `u`.`name` " +
				"HAVING SUM(`p`.`amount`) > 5000 AND COUNT(`p`.`id`) < 100 " +
	...
	t.Run("bool as TRUE/FALSE", func(t *testing.T) {
		t.Parallel()
		expected := sqltesting.Compact(
	...
	t.Run("subquery in FROM", func(t *testing.T) {
		...
		expected := sqltesting.Compact(
			"SELECT `sub`.`user_id`, `sub`.`total` " +
				"FROM (SELECT `user_id`, SUM(`amount`) AS `total` FROM `orders` GROUP BY `user_id`) AS `sub` " +
...
func TestSqblMysql_EntryPoints(t *testing.T) {
	t.Parallel()
	...
	t.Run("DropIndex", func(t *testing.T) {
		t.Parallel()
		got := DropIndex("idx_users_email").On("users").ToSql()
	...
	t.Run("OnDuplicateKeyUpdate", func(t *testing.T) {
		t.Parallel()
		got := InsertInto("tags").
			Columns("name", "count").
		...
		if got != want {
			t.Errorf("OnDuplicateKeyUpdate: got %q, want %q", got, want)
		}
	})
}
